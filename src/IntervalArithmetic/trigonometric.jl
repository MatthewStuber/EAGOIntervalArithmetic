...
half_pi(::Type{T}) where T = pi_interval(T) / 2.0
half_pi(x::T) where T<:AbstractFloat = half_pi(T)

const pih64 = pi_interval(Float64)/Float64(2.0)
const pih32 = pi_interval(Float32)/Float32(2.0)
const pih16 = pi_interval(Float16)/Float16(2.0)

const pi264 = pi_interval(Float64)*Float64(2.0)
const pi232 = pi_interval(Float32)*Float32(2.0)
const pi216 = pi_interval(Float16)*Float16(2.0)

two_pi(::Type{Float64}) = pi264
two_pi(::Type{Float32}) = pi232
two_pi(::Type{Float16}) = pi216

range_atan2(::Type{T}) where {T<:AbstractFloat} = MCInterval{T}(-(pi_interval(T).hi), pi_interval(T).hi)
half_range_atan2(::Type{T}) where {T} = (temp = half_pi(T); Interval(-(temp.hi), temp.hi) )
pos_range_atan2(::Type{T}) where {T<:AbstractFloat} = MCInterval{T}(zero(T), pi_interval(T).hi)
...
function tan(a::MCInterval{T}) where {T<:AbstractFloat}
    ...
    diam(a) > pi_interval(T).lo && return entireinterval(T)
